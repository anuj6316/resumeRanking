...
import re
import spacy
import utils.total_exp_years as exp_years
import datetime
...
def extract_section(raw_text, section):
    ...
    in_section = False
    for line in cv_list:
        if not in_section and any(
            kw in line.lower() for kw in SECTION_KEYWORDS[section]
        ):
            in_section = True
        ...
        if in_section and any(
            kw in line.lower()
            for kw in sum(SECTION_KEYWORDS.values(), [])
            if kw not in SECTION_KEYWORDS[section]
        ):
            break
        if in_section:
            section_lines.append(line)
